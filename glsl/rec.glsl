...
vec2 getUv(){
    ...
    if(u_resolution.x>u_resolution.y){
        ...
        xy.x+=(u_resolution.y-u_resolution.x)/u_resolution.y/2.;
    }else{
        xy.y*=u_resolution.y/u_resolution.x;
        xy.y+=(u_resolution.x-u_resolution.y)/u_resolution.x/2.;
    }
    xy-=.5;
    return xy;
}
...
vec3 pRotate(vec3 p){
    mat4 matrix=rotationAxis(
        RAD*360.*u_time*.2+st.x*st.y,
        vec3(0.,1.,0.)
    );
    return p=(vec4(p,1.)*matrix).xyz;
}
float vmax(vec3 v){
...
vec3 getNormal(in vec3 p){
    ...
    Camera getCamera(vec3 position,vec3 target){
        ...
        camera.right=normalize(vec3(camera.forward.z,0.,-camera.forward.x));
    ...
    Surface getSurface(Marcher marcher){
        ...
        Surface surface=Surface(position,normal,vec3(0.));
        return surface;
    }
    const int OSTEPS=4;
    ...
    Light getLight(
        ...
        const float specularity=16.;
        Light light=Light(color,position,vec3(0.),vec3(0.),0.,0.,0.,0.,1.);
        light.direction=light.position-surface.position;
        light.distance=length(light.direction);
        light.direction/=light.distance;
        light.attenuation=min(1./(.25*light.distance*light.distance),1.);// Keeps things between 0 and 1.
        light.reflected=reflect(-light.direction,surface.normal);
        light.diffuse=max(0.,dot(surface.normal,light.direction));
        light.specular=max(0.,dot(light.reflected,normalize(camera.position-surface.position)));
    ...
    vec3 render(){
        ...
        return clamp(surface.rgb,0.,1.);// from 0 to 1
    }
